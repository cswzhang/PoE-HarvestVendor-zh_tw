...
#Include %A_ScriptDir%\processText.ahk
...
Handle_Reforge(craftText, ByRef out) {
    ;prefixes
    if InStr(craftText, "前綴") > 0 {
        if InStr(craftText, "幸運") > 0 {
    ...
    remAddsStr := ["法術", "物理", "火焰", "攻擊", "生命", "冰冷"
        , "速度", "防禦", "閃電", "混沌", "暴擊", "勢力"]
    if (InStr(craftText, "包含") > 0) { ; or InStr(craftText, "orRare") > 0) { ; 'new random' text appears only in reforge rares
        for k, v in remAddsClean {
            if (InStr(craftText, remAddsStr[k]) > 0) {
                if (InStr(craftText, "容易") > 0 ) {
    ...
    ;} 
    ;reforge same mod
    if (InStr(craftText, "更低機率") > 0) {
        out.push(["Reforge Rare Less Likely"
            , getLVL(craftText)
            , "Other"])
        return
    }
    if (InStr(craftText, "更高機率") > 0) {
    ...
    ;links
    if (InStr(craftText, "相連的插槽") > 0 and InStr(craftText, "10次") = 0) { 
        if InStr(craftText,"6") > 0 {
            out.push(["Six Links"
                , getLVL(craftText)
                , "Other"])
        } else if InStr(craftText, "5") > 0 {
            out.push(["Five Links"
                , getLVL(craftText)
                , "Other"])
        }
    ...
    ;colour
    if (InStr(craftText, "顏色") > 0 and InStr(craftText, "10次") = 0) {
        reforgeNonColor := ["Non-Red", "Non-Blue", "Non-Green"]
        reforgeNonColorText := ["非紅色", "非藍色", "非綠色"]
        for k, v in reforgeNonColor {
            if InStr(craftText, reforgeNonColorText[k]) > 0 {
        ...
        reforge2colorText := ["紅色和藍色", "紅色和綠色", "藍色和綠色"
            , "紅色、藍色和綠色", "白色"]
        for k, v in reforge2color {
            if InStr(craftText, reforge2colorText[k]) > 0 {
